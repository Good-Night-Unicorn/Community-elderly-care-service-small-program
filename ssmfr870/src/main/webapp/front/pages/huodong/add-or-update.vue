<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' v-model="ruleForm.shijian" placeholder="时间" @tap="toggleTab('shijian')"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`huodong`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`huodong`, this.ruleForm);
				}else{
					await this.$api.add(`huodong`, this.ruleForm);
				}
